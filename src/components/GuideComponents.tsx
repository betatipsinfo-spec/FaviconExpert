  ...
  return (
    <div className="space-y-12">
      {steps.map((step, i) => (
        ...
        >
          <div className="w-12 h-12 rounded-2xl bg-white/5 border border-white/5 flex items-center justify-center flex-shrink-0 text-brand-purple group-hover:bg-brand-purple/10 transition-all">
            ...
            {step.code && (
              <div className="relative group/code">
                <div className="absolute top-4 right-4 opacity-0 group-hover/code:opacity-100 transition-opacity">
                  ...
                  >
                    {copiedIndex === i ? <Check className="w-4 h-4 text-green-400" /> : <Copy className="w-4 h-4 text-slate-400" />}
                  </button>
                </div>
                <pre className="bg-brand-dark/50 border border-white/5 p-6 rounded-2xl overflow-x-auto text-brand-cyan font-mono text-xs">
                  <code>{step.code}</code>
                </pre>
              </div>
            )}
        ...
        <tbody className="text-sm">
          {specs.map((spec, i) => (
            <tr key={i} className="border-t border-white/5 hover:bg-white/[0.02] transition-all">
              <td className="px-8 py-6 font-mono text-brand-cyan">{spec.name}</td>
              <td className="px-8 py-6 text-slate-300">{spec.size}</td>
              <td className="px-8 py-6">
                <span className="px-2 py-1 rounded bg-white/10 text-[10px] font-bold text-slate-400">{spec.type}</span>
              </td>
              <td className="px-8 py-6 text-slate-500">{spec.purpose}</td>
            </tr>
          ))}
        </tbody>
      </table>
